...
@section('content')
    ...
    <div class=" mt-3 mx-5">
        <h1 class="fs-4"> ( {{$color->name}} ) has {{$products->total()}} Products </h1>
        <hr>
        ...
        <table class="table table-hover table-sm table-light  shadow  ">
            ...
            @if ($products->isNotEmpty())
                ...
                @foreach($products as $product)
                    <tr valign='middle' >
                        ...
                        <td class="table-success">{{$product->color->name}}<div class="color" style="background-color:{{$product->color->hex}};"></div></td>
            ...
            @else
                <tr class="table-danger" align="center" >
                    <td colspan="11"  class="fs-2"  > Data Not Found</td>
                </tr>
            @endif
            </tbody>
        </table>
        ...
        <a href="{{route('colors.list')}}" class="btn btn-primary">colors list</a>
