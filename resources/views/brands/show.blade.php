...
@section('content')
    ...
    <div class=" mt-3 mx-5">
        <h1 class="fs-4"> ( {{$brand->name}} ) has {{$products->total()}} Products </h1>
        <hr>
            ...
            @if ($products->isNotEmpty())
                ...
                @foreach($products as $product)
                    <tr valign='middle' >
                        ...
                        <td class="table-success">{{$product->brand->name}}</td>
                        <td>{{$product->stock}}</td>
        ...
        <a href="{{route('brands.list')}}" class="btn btn-primary">brands list</a>
