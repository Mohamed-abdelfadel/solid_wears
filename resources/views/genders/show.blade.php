...
@section('content')
    ...
    <div class=" mt-3 mx-5">
        <h1 class="fs-4"> ( {{$gender->name}} ) has {{$products->total()}} Products </h1>
        <hr>
            ...
            @if ($products->isNotEmpty())
                ...
                @foreach($products as $product)
                    <tr valign='middle' >
                        ...
                        <td>{{$product->price . ' '}}$</td>
                        <td>{{$product->cost . ' '}}$</td>
                        <td class="table-success">{{$product->gender->name}}</td>
        ...
        <a href="{{route('genders.list')}}" class="btn btn-primary">genders list</a>
