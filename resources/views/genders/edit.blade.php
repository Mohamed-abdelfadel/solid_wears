...
@section('content')
    ...
    <div class="mx-5">
        <div class="row">
            <div class="col-6">
                <h1 class="fs-2 mt-4">New gender</h1>
                <hr>
                <div class="row">
                <form class="col-6" action="{{route('genders.update' ,  $gender->id )}}" method="post">
                    @csrf
                    @method('PUT')
                        <div class="form-floating mb-4 ">
                            <input type="text" class="form-control   @error('name') is-invalid @enderror " name="name" id='name' placeholder="name" value="{{ old('name',$gender->name) }}">
                            <label for="name" class="mx-3">gender name:</label>
                            @error('name')
                            <p class="invalid-feedback fs-6"> {{ $message }} </p>
                            @enderror
                        </div>
                        <button type="submit" class="btn btn-success px-4 " name="button">Update</button>
                        <a href="{{route('genders.list')}}" class="btn btn-outline-primary px-4">Customers list</a>
                </form>
            ...
            <div class="col-6">
                <h1 class="fs-2 mt-4">genders list</h1>
                <hr>
